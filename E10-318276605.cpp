...
//NEW// Keyframes
float posXavion = 2.0, posYavion = 5.0, posZavion = -3.0;  //posicion inicial del helicoptero
float	movAvion_x = 0.0f, movAvion_y = 0.0f; //las variables que se mueven
float giroAvion = 0;
...
void saveFrame(void) //tecla L
{
	...
	FILE* file;
	errno_t err = fopen_s(&file, "keyframes.txt", "a");

	if (err == 0 && file != NULL) {
		fprintf(file, "KeyFrame[%d].movAvion_x = %ff;\n", FrameIndex - 1, movAvion_x);
		fprintf(file, "KeyFrame[%d].movAvion_y = %ff;\n", FrameIndex - 1, movAvion_y);
		fprintf(file, "KeyFrame[%d].giroAvion = %f;\n\n", FrameIndex - 1, giroAvion);
		fclose(file);
	}
	else {
...
void animate(void)
{
	...
	if (play)
	{
		if (i_curr_steps >= i_max_steps) //Identifica si ya se llegó al ultimo cuadro //fin de animación entre frames?
		{
			playIndex++;
			printf("playindex : %d\n", playIndex);
